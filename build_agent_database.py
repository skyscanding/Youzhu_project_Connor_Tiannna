...
def build(source_root: Path, output: Path, supplemental_docx_paths: list[Path] | None = None, supplemental_text_path: Path | None = None) -> None:
    ...
    # Correct known national standards whose legacy import labels describe how
    # we obtained a file, rather than who officially issued the standard.
    target.execute(
        """UPDATE documents
           SET issuing_authority=?, document_number=?, published_date=?, effective_date=?,
               validity_status=?, official_url=?
           WHERE title_normalized=?""",
        (
            "住房和城乡建设部",
            "GB 55002-2021；住房和城乡建设部公告2021年第61号",
            "2021-04-09",
            "2022-01-01",
            "现行有效",
            "https://jshbj.xjbt.gov.cn/c/2021-11-08/8175501.shtml",
            normalise_title("建筑与市政工程抗震通用规范"),
        ),
    )
